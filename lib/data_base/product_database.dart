...
class ProductDatabase
{
  ...
  Future<List<DocumentSnapshot>> getallproducts(String CAT) async
  {
    ...
    if ((CAT != null && CAT != "All"))
    {
      if (CAT != "All" )
      {
        QuerySnapshot querytSnapshot = await Firestore.instance.collection("products")
      ...
      {
        QuerySnapshot querytSnapshot = await Firestore.instance.collection("products").getDocuments();
        documentSnapshot = querytSnapshot.documents;
      }
    }
    else
  ...
  Future<List<DocumentSnapshot>> getSuggestions(String pattern) async
  {
    ...
    QuerySnapshot querytSnapshot = await _firestore.collection("products").getDocuments();
    documentSnapshot = querytSnapshot.documents;
